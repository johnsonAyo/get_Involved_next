...
import { fetchLocalGovernments, fetchStates } from "../data/directoryOptions.js";
import { fetchParties } from "../data/parties.js";
import { fetchPositions } from "../data/positions.js";
...
export function SubmitCandidatePage({ onNavigate }: { onNavigate: (path: string) => void }) {
  const [form, setForm] = useState(initialForm);
  const [validationError, setValidationError] = useState<string | null>(null);
  const optionQueries = useQueries({
    queries: [
      { queryKey: ["positions"], queryFn: fetchPositions },
      { queryKey: ["states"], queryFn: fetchStates },
      { queryKey: ["parties"], queryFn: fetchParties },
    ],
  });
  const [positionsQuery, statesQuery, partiesQuery] = optionQueries;
  const { data: localGovernments = [], isFetching: isLoadingLocalGovernments } = useQuery({
    queryKey: ["local-governments", form.state],
    queryFn: () => fetchLocalGovernments(form.state),
    enabled: Boolean(form.state),
  });
          ...
          {submission.isSuccess ? (
            ...
            <>
              ...
              <form className="ds-form submit-page__form" onSubmit={handleSubmit}>
                ...
                <div className="ds-field">
                  ...
                  <div className="ds-field__party-row">
                    <DropdownSelect
                      ...
                      variant="field"
                    />
                    {(partiesQuery.data ?? []).find((party) => party.id === form.party)?.logo ? (
                      <div className="ds-field__party-logo">
                        <img
                          alt="Selected logo indicator"
                          src={(partiesQuery.data ?? []).find((party) => party.id === form.party)?.logo}
                        />
                      </div>
                    ) : null}
                  </div>
                ...
                <SelectField
                  disabled={!form.state || isLoadingLocalGovernments}
